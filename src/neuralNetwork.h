...
public:
    ...
    template <typename Derived>
    double lookup_ngram(const Eigen::MatrixBase<Derived> &ngram, std::vector<double> &output_dist)
    {
		assert (ngram.rows() == m->ngram_size);
		assert (ngram.cols() == 1);
		...
		if (normalization)
		{
		    ...
		    double logz = logsum(scores.col(0));
		    log_prob = weight * (scores(output, 0) - logz);
			for (int index = 0; index < m->output_vocab_size; index++) {
				output_dist[index] = weight * (scores(index, 0) - logz);
			}
		}
		else
		{

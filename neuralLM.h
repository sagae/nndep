...
public:
	...
	//Return not only the logprob but also the probability distribution over the output symbols
    double lookup_ngram(const int *ngram_a, int n, std::vector<double> &output_dist)
    {
        Eigen::Matrix<int,Eigen::Dynamic,1> ngram(m->ngram_size);
		for (int i=0; i<m->ngram_size; i++)
		{
		...
		return neuralNetwork::lookup_ngram(ngram, output_dist);
    }
    ...
    double lookup_ngram(const std::vector<int> &ngram_v, std::vector<double> &output_dist)
    {
        return lookup_ngram(ngram_v.data(), ngram_v.size(), output_dist);
    }
	
	template <typename DerivedB>
    double lookup_ngram(const std::vector<int> &ngram_v, const Eigen::MatrixBase<DerivedB> &log_probs_const)
    {
        return lookup_ngram(ngram_v.data(), ngram_v.size());
    }

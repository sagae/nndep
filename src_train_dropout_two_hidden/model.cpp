	...
	int context_vector_size)
{
	...
	first_hidden_linear_context.resize(num_hidden, context_vector_size);
	...
	this->num_second_hidden = num_second_hidden;	
    premultiplied = false;
}
    ...
    double adagrad_epsilon)
{
    ...
    first_hidden_linear.initialize(init_engine,
        ...
        adagrad_epsilon);
	/*	
    first_hidden_linear_context.initialize(init_engine,
        init_normal,
        init_range,
        parameter_update,
        adagrad_epsilon);		
...
void model::readConfig(ifstream &config_file)
{
	...
	else if (fields[0] == "context_vector_size")
	    context_vector_size = lexical_cast<int>(fields[1]);	
	else if (fields[0] == "num_hidden")
	    num_hidden = lexical_cast<int>(fields[1]);
	else if (fields[0] == "num_second_hidden")
	    num_second_hidden = lexical_cast<int>(fields[1]);
    ...
    resize(ngram_size,
        ...
        input_embedding_dimension,
        num_hidden,
		num_second_hidden,///VERY IMPORTANT!! THIS VALUE NEEDS TO BE CHANGED
        output_embedding_dimension,
		context_vector_size);
...
//Scaling for dropout. Right now, only the linear layers are being scaled
void model::scale (double hidden1_scaling_constant,
					double hidden2_scaling_constant,
					double hidden3_scaling_constant) {
	first_hidden_linear.scale(hidden1_scaling_constant);
	second_hidden_linear.scale(hidden2_scaling_constant);
	//first_hidden_linear_context.scale(hidden1_scaling_constant);
}
void model::read(const string &filename, vector<string> &input_words, vector<string> &output_words)
{
	...
	//else if (line == "\\context_weights")
	//	first_hidden_linear_context.read_weights(file);
	//else if (line == "\\context_biases")
	//	first_hidden_linear_context.read_biases(file);
...
void model::write(const string &filename, const vector<string> *input_pwords, const vector<string> *output_pwords)
{
	...
	file << "context_vector_size" << context_vector_size;

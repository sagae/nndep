...
class Activation_function
{
	...
	void fPropDropout(const MatrixBase<DerivedIn> &input, 
		const MatrixBase<DerivedOut> &output,
		const MatrixBase<DerivedBool> &dropout_mask)
    const {
	    UNCONST(DerivedOut, output, my_output);
		    /*
			std::cerr<<"output size is "<<output.rows()<<" "<<output.cols()<<std::endl;
			std::cerr<<"input size is "<<input.rows()<<" "<<input.cols()<<std::endl;
			std::cerr<<"dropout_mask size is "<<dropout_mask.rows()<<" "<<dropout_mask.cols()<<std::endl;
			*/
		    switch (f)
		    {
			    case Identity: my_output = input.array()*dropout_mask.array(); break;
			    case Rectifier: my_output = input.unaryExpr(rectifier_functor()).array()*dropout_mask.array(); break;
			    case Tanh: my_output = input.unaryExpr(tanh_functor()).array()*dropout_mask.array(); break;
			    case HardTanh: my_output = input.unaryExpr(hardtanh_functor()).array()*dropout_mask.array(); break;
		    }
    }
    ...
    template <typename DerivedGOut, typename DerivedGIn, typename DerivedIn, typename DerivedOut, typename DerivedBool>
	void bPropDropout(const MatrixBase<DerivedGOut> &input, 
      MatrixBase<DerivedGIn> &output,
		   const MatrixBase<DerivedIn> &finput,
       const MatrixBase<DerivedOut> &foutput,
	   const MatrixBase<DerivedBool> &dropout_mask) const
        {
	    UNCONST(DerivedGIn, output, my_output);
		    ...
		    {
			    case Identity: my_output = input; break;
			    case Rectifier: my_output = finput.array().unaryExpr(drectifier_functor()) * input.array()*dropout_mask.array(); break;
			    case Tanh: my_output = foutput.array().unaryExpr(tanh_functor()) * input.array()*dropout_mask.array(); break;
			    case HardTanh: my_output = finput.array().unaryExpr(hardtanh_functor()) * input.array()*dropout_mask.array(); break;
		    }
    }
};

...
void readContextVectorsFile(const std::string &filename, int &context_size, std::vector<double> &data, int minibatch_size=0);
...
void writeVector(std::vector<datatype> &param, const std::string &filename){
    std::cerr << "Writing parameters to " << filename << std::endl;
    ...
    for (int line= 0;line< param.size();line++){
    	OUT<<param[line]<<std::endl;
	}
    OUT.close();
}
...
{
    //std::cerr<<"number of rows is "<<param.rows()<<std::endl;
	//std::cerr<<"number of cols is "<<param.cols()<<std::endl;

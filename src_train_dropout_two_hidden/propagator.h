...
public:
	...
	Node<Linear_layer> first_hidden_linear_context_node;
...
public:
	...
	first_hidden_linear_context_node(&nn.first_hidden_linear_context, minibatch_size),
    ...
    void resize(int minibatch_size) {
	  ...
	  first_hidden_linear_context_node.resize(minibatch_size);
  ...
  //std::getchar();
    ...
    template <typename Derived>
    void fPropContext(const MatrixBase<Derived> &data)
    {
        if (!pnn->premultiplied)
	{
            start_timer(0);
    ...
    //first_hidden_linear_context_node.param->fProp(context_vectors, 
	//				  first_hidden_linear_context_node.fProp_matrix);
  ...
  //std::getchar();
    ...
    template <typename Derived, typename DerivedBool>
    void fPropDropout(const MatrixBase<Derived> &data,
					  const MatrixBase<DerivedBool> &first_hidden_dropout_mask,
					  const MatrixBase<DerivedBool> &second_hidden_dropout_mask)
    {
        if (!pnn->premultiplied)
	{
            start_timer(0);
	...
	first_hidden_activation_node.param->fPropDropout(first_hidden_linear_node.fProp_matrix,
						  first_hidden_activation_node.fProp_matrix,
  ...
  //std::getchar();
	...
	second_hidden_linear_node.param->fProp(first_hidden_activation_node.fProp_matrix,
					       second_hidden_linear_node.fProp_matrix);
	second_hidden_activation_node.param->fPropDropout(second_hidden_linear_node.fProp_matrix,
						   second_hidden_activation_node.fProp_matrix,
    ...
    template <typename DerivedIn, typename DerivedOut, typename DerivedBool>
  ...
  if (parameter_update == "SGD") {
    ...
    template <typename DerivedIn, typename DerivedOutI, typename DerivedOutV, typename DerivedBool>
    void bProp(const MatrixBase<DerivedIn> &data,
...
private:
    template <typename DerivedIn, typename DerivedBool>
  ...
  ////////BACKPROP////////////
        start_timer(9);
	  	...
	  	second_hidden_activation_node.param->bPropDropout(output_layer_node.bProp_matrix,
		...
		start_timer(11);
		first_hidden_activation_node.param->bPropDropout(second_hidden_linear_node.bProp_matrix,
	  ...
	  //first_hidden_linear_context_node.param->bProp(first_hidden_activation_node.bProp_matrix,
	  //					      first_hidden_linear_context_node.bProp_matrix);
		stop_timer(11);
	  ...
	  if (parameter_update == "SGD") {
       ...
       stop_timer(10);
	    ...
	    //first_hidden_linear_context_node.param->computeGradient(first_hidden_activation_node.bProp_matrix,
	    //            context_vectors,
	    //            learning_rate, momentum, L2_reg);
	    stop_timer(12);

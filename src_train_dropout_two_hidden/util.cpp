...
// Read context vectors of unknown size into a flat vector<double>.
// If this takes too much memory, we should create a vector of minibatches.
void readContextVectorsFile(const string &filename, int &context_size, vector<double> &data, int minibatch_size)
{
  cerr << "Reading context vectors from file " << filename << ": ";
  ...
  while (getline(DATAIN, line))
  {
    ...
    if (context_size == 0)
        context_size = ngram.size();

    if (ngram.size() != context_size)
    {
        cerr << "Error: expected " << context_size << " fields in instance, found " << ngram.size() << endl;
	exit(-1);
    }

    for (int i=0;i<context_size;i++)
        data.push_back(boost::lexical_cast<double>(ngram[i]));
